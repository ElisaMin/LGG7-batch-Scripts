...
echo.
echo -------------------------------------------------------------------------------
choice /c abc /t 7 /d c /m "三秒钟后默认全部刷入"
echo -------------------------------------------------------------------------------
if %errorlevel%==1 %fastboot% flash boot_a %boot% && %fastboot% reboot 
if %errorlevel%==2 %fastboot% flash boot_b %boot% && %fastboot% reboot 
if %errorlevel%==3 %fastboot% flash boot_a %boot% && %fastboot% flash boot_b %boot% && %fastboot% reboot 
goto end
:slot-active
echo 请选择要激活的系统槽^/分区
echo -------------------------------------------------------------------------------
